...
public class RarePlasticWall extends BlockWall
{
	public static Block BlockType;

    public RarePlasticWall(Block p_i45435_1_)
    {
        super(p_i45435_1_);
	...
	public boolean canConnectWallTo(IBlockAccess p_150091_1_, int p_150091_2_, int p_150091_3_, int p_150091_4_)
	{
		...
		return block != this && block != Blocks.fence_gate  && block != PlasticCore.PlasticWall && block != Blocks.wall_sign && block != PlasticCore.PlasticFenceGate  && block != PlasticCore.RarePlasticFenceGate ? (block.getMaterial().isOpaque() && block.renderAsNormalBlock() ? block.getMaterial() != Material.gourd : false) : true;
	}
}