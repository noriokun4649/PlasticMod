...
public class PlasticTeleporter extends Teleporter {
	...
	public PlasticTeleporter(WorldServer par1WorldServer) {
		...
		if (dim == 0) {
			thePlayer.mcServer.getConfigurationManager()
					.transferPlayerToDimension(
							thePlayer,
							0,
							new PlasticTeleporter(thePlayer.mcServer
									.worldServerForDimension(0)));
			return true;
		} else if (dim == PlasticConfig.dimensionID) {
			thePlayer.mcServer
					.getConfigurationManager()
					.transferPlayerToDimension(
							thePlayer,
							PlasticConfig.dimensionID,
							new PlasticTeleporter(
									thePlayer.mcServer
											.worldServerForDimension(PlasticConfig.dimensionID)));
			return true;
		}
		return false;
			...
			for (int l = -2; l <= 2; ++l) {
				for (int i1 = -2; i1 <= 2; ++i1) {
					for (int j1 = -1; j1 < 3; ++j1) {
						...
						boolean flag = j1 < 0;
                        if(flag) {
                            this.worldServerInstance.setBlock(k1, l1, i2, PlasticCore.plasticblock);
                        }
                        else {
	...
	public boolean placeInExistingPortal(Entity par1Entity, double par2,
			...
			for (k1 = l - short1; k1 <= l + short1; ++k1) {
				...
				for (int l1 = i1 - short1; l1 <= i1 + short1; ++l1) {
					...
					for (int i2 = this.worldServerInstance.getActualHeight() - 1; i2 >= 0; --i2) {
						if (this.worldServerInstance.getBlock(k1, i2, l1) == PlasticCore.plasticportalblock) {
							while (this.worldServerInstance.getBlock(k1,
									i2 - 1, l1) == PlasticCore.plasticportalblock) {
								--i2;
							}
		...
		if (d3 >= 0.0D) {
			...
			if (this.worldServerInstance.getBlock(i - 1, j, k) == PlasticCore.plasticportalblock) {
				j2 = 2;
			}
			...
			if (this.worldServerInstance.getBlock(i, j, k - 1) == PlasticCore.plasticportalblock) {
				j2 = 3;
			}
			...
			for (k2 = -1; k2 <= 1; ++k2) {
				for (i3 = 1; i3 < 3; ++i3) {
					for (l2 = -1; l2 < 3; ++l2) {
						...
						flag = l2 < 0;
                        if(flag) {
                            this.worldServerInstance.setBlock(k3, j3, i4, PlasticCore.plasticblock);
                        }
                        else {
			...
			for (i3 = 0; i3 < 4; ++i3) {
				for (l2 = -1; l2 < 4; ++l2) {
					...
					this.worldServerInstance.setBlock(k3, j3, i4,
							flag ? PlasticCore.plasticblock
									: PlasticCore.plasticportalblock, 0, 2);
				}
			}
